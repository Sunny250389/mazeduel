    ...
    <div style={styles.container}>
      ...
      {mazes.map((m, i) => (
        <div key={i} style={styles.card}>
          <div>
            <b style={{ color:"#00ff88" }}>{m.difficulty?.toUpperCase()} {m.width}×{m.height}</b>
            <span style={{ color:"#888", marginLeft:10, fontSize:12 }}>{m.date}</span>
          </div>
          <div style={{ color:"#ffd700" }}>Best: {m.bestEff}%</div>
          <div style={styles.row}>
            <button style={styles.btn} onClick={() => onPlay(m)}>▶ Play Again</button>
            <button style={styles.ghostBtn}
              onClick={() => { navigator.clipboard.writeText(encodeMaze(m)); alert("Code copied!"); }}>
              📋 Share Code
            </button>
          </div>
        </div>
...
const styles = {
  ...
  title:     { color:"#00ff88" },
  card:      { background:"#16213e", border:"1px solid #0f3460", borderRadius:10,
               padding:16, marginBottom:12, maxWidth:400 },
  row:       { display:"flex", gap:8, marginTop:10 },
  btn:       { padding:"8px 16px", background:"#00ff88", color:"#000",
               border:"none", borderRadius:6, cursor:"pointer", fontWeight:"bold" },
  ghostBtn:  { padding:"8px 12px", background:"transparent", color:"#aaa",
               border:"1px solid #333", borderRadius:6, cursor:"pointer" },
  backBtn:   { marginTop:20, background:"transparent", color:"#555", border:"none",
